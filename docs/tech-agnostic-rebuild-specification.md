...
**Implications**:
- Statistical correlation analysis (e.g., Spearman's ρ)
...
- Correlation coefficient (ρ): -1 to +1 range
...
**Confidence Levels**:
- High: p-value < 0.01, sample size ≥ 20
- Medium: p-value < 0.05, sample size ≥ 10
- Low: p-value < 0.05, sample size ≥ 5

### Flare Lifecycle Rules

**Status Transitions**:
- Active → Improving: user marks trend as improving
- Active → Worsening: user marks trend as worsening
- Improving → Worsening: allowed (flare can regress)
- Worsening → Improving: allowed (flare can improve)
- Any status → Resolved: user marks as resolved
- Resolved → Any: NOT allowed (resolved is final state)
...
**Steps**:
1. Navigate to "Export" or "Settings → Data"
...
**Capabilities**:
- Create flare:
  - From body map (tap region → zoom → tap location)
...
- Correlation analysis:
  - Automatic statistical correlation (Spearman's ρ)
...
**Example**:
- Tap body map → marker appears immediately
- Severity slider → number updates in real-time
...
**Note**: ⭐ denotes groin-specific regions added for HS tracking (primary requirement from PRD).
...
**Why Spearman's ρ (not Pearson's r)**:
...
1. **Time-Series Alignment**:
   ...
   - Example: Food at 12:00 PM, lag = 2hr, window = 4hr → symptoms 2:00-6:00 PM

2. **Pair Creation**:
   - Create (food_portion, symptom_severity) pairs
   - Multiple symptom instances in window → multiple pairs
   - No symptom in window → exclude this food event
...
5. **P-Value Calculation**:
   - t-statistic: t = ρ * sqrt((n-2) / (1-ρ²))
   - Degrees of freedom: df = n - 2
   - Two-tailed t-test to get p-value
   - Significance threshold: p < 0.05

6. **Confidence Level**:
   - High: p < 0.01 AND n ≥ 20
   - Medium: p < 0.05 AND n ≥ 10
   - Low: p < 0.05 AND n ≥ 5
...
2. Identify lag with highest |ρ| (absolute value)
...
2. For each combination of 2+ foods:
   ...
   - Compare: synergistic if `ρ_combination > max(ρ_individual) + 0.15`

**Example**:
- Tomato alone: ρ = 0.30 (p < 0.05, n=15)
- Egg alone: ρ = 0.25 (p < 0.05, n=12)
- Tomato + Egg: ρ = 0.62 (p < 0.01, n=10)
...
**Unit Tests**:
- Correlation algorithm (verify Spearman's ρ calculation)
- Data validation (severity ranges, timestamp formats)
- Encryption/decryption (verify AES-256-GCM)

**Integration Tests**:
- Flare lifecycle workflows (create → update → resolve)
