...
import { generatePowerUserData } from "@/lib/dev/generators/generatePowerUserData";
...
export function DevDataControls() {
  ...
  const handleGeneratePowerUserData = async (years: number) => {
    if (!userId) {
    ...
    try {
      const result = await generatePowerUserData({ userId, years });

      setMessage(
        `✅ Generated ${years} year(s) of power user data!\\n\\n` +
        `📊 **Entities Created:**\\n` +
        `• ${result.symptomsCreated} symptoms\\n` +
        `• ${result.medicationsCreated} medications\\n` +
        `• ${result.triggersCreated} triggers\\n` +
        ...
        `• ${result.foodEvents} food events\\n\\n` +
        `🗓️ **Daily Tracking:**\\n` +
        `• ${result.dailyLogsCreated} daily logs (${result.dailyLogCoverage.toFixed(0)}% coverage)\\n\\n` +
        `🔬 **Analytics:**\\n` +
        `• ${result.correlationsGenerated} correlations\\n` +
        `• ${result.patternsGenerated} intentional patterns\\n\\n` +
        `📅 **Date Range:** ${result.startDate} to ${result.endDate} (${result.daysGenerated} days)\\n\\n` +
        `**Where to view:**\\n` +
        `• Dashboard: See flares and quick stats\\n` +
        `• Daily Log: View mood + sleep trends\\n` +
        `• Body Map: See flare locations\\n` +
        `• Timeline: Browse all health events\\n` +
        `• Insights: Explore correlations\\n\\n` +
        `**Refresh the page to see your data!**`
      );
    } catch (err) {
      console.error("Failed to generate power user data", err);
      setError(
    ...
    <div className="mt-10 space-y-6">
      ...
      <div className="rounded-lg border-2 border-dashed border-purple-500/60 bg-purple-500/10 p-6">
        ...
        <div className="mt-6">
          <button
            type="button"
            onClick={() => handleGeneratePowerUserData(selectedYears)}
            disabled={isLoading}
