...
describe("ActiveFlareCards", () => {
  ...
  const createMockFlare = (
    id: string,
    overrides: Partial<ActiveFlare & { trend: "worsening" | "stable" | "improving" }> = {}
  ): ActiveFlare & { trend: "worsening" | "stable" | "improving" } => ({
    id,
    userId: mockUserId,
    symptomId: "symptom-1",
    symptomName: `Test Flare ${id}`,
  ...
  describe("AC1: Displays 0-5 active flare cards", () => {
    ...
    it("should render one flare card", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText("Test Flare flare-1")).toBeInTheDocument();
      });
    });

    it("should render three flare cards", async () => {
      const mockFlares = [
        createMockFlare("flare-1"),
      ...
      await waitFor(() => {
        expect(screen.getByText("Test Flare flare-1")).toBeInTheDocument();
        expect(screen.getByText("Test Flare flare-2")).toBeInTheDocument();
        expect(screen.getByText("Test Flare flare-3")).toBeInTheDocument();
      });
    });
    ...
    it("should render exactly 5 flare cards when there are 5 flares", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText("Test Flare flare-1")).toBeInTheDocument();
        expect(screen.getByText("Test Flare flare-5")).toBeInTheDocument();
      });
    });
    ...
    it("should show only 5 flares when there are more than 5", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText("Showing 5 of 7 active flares")).toBeInTheDocument();
      });
    });
  });
  ...
  describe("AC2: Each card shows body location, duration, severity, trend arrow", () => {
    it("should display body location", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText("Lower Back, Hip")).toBeInTheDocument();
      });
    });
  ...
  describe("AC5: Trend arrows with color coding", () => {
    it("should use red color for worsening trend", async () => {
      ...
      await waitFor(() => {
        const trendIcon = screen.getByLabelText("Worsening");
        expect(trendIcon.parentElement).toHaveClass("text-red-600");
      });
    });
    ...
    it("should use green color for improving trend", async () => {
      ...
      await waitFor(() => {
        const trendIcon = screen.getByLabelText("Improving");
        expect(trendIcon.parentElement).toHaveClass("text-green-600");
      });
    });
  });
  ...
  describe("AC8: Resolve button shows confirmation and resolves flare", () => {
    it("should show confirmation dialog when Resolve is clicked", async () => {
      ...
      expect(global.confirm).toHaveBeenCalledWith(
        expect.stringContaining('Are you sure you want to resolve "Test Flare flare-1"?')
      );
    });
  ...
  describe("AC9: Sorting functionality", () => {
    it("should sort by severity (highest first) by default", async () => {
      ...
      await waitFor(() => {
        ...
        expect(flareNames[0]).toHaveTextContent("High Severity");
        expect(flareNames[1]).toHaveTextContent("Mid Severity");
        expect(flareNames[2]).toHaveTextContent("Low Severity");
      });
    });
    ...
    it("should sort by recency when recency button is clicked", async () => {
      ...
      const flareNames = screen.getAllByRole("heading", { level: 3 });
      expect(flareNames[0]).toHaveTextContent("Newest");
      expect(flareNames[1]).toHaveTextContent("Middle");
      expect(flareNames[2]).toHaveTextContent("Oldest");
    });
  ...
  describe("Accessibility", () => {
    it("should have proper ARIA labels on buttons", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByLabelText("Update Test Flare flare")).toBeInTheDocument();
        expect(screen.getByLabelText("Resolve Test Flare flare")).toBeInTheDocument();
      });
    });
